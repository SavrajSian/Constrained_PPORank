...
def main(Debug=False):
    ...
    if args.algo == 'ppo':
        ...
        if args.distributed:
            obs_critic_shape = [M, agent.module.actor_critic.critic_size]
        else:
        ...
        if args.resume:
            ...
            if args.distributed:
                agent.module.policy_net.load_state_dict(checkpoint['policy_state_dict'])
                agent.module.value_net.load_state_dict(checkpoint['value_state_dict'])
                agent.module.optimizer.load_state_dict(checkpoint['optimizer'])
            else:
        ...
        for epoch in range(epochs):
            ...
            if args.use_linear_lr_decay:
                if args.distributed:
                    utils.update_linear_schedule(agent.module.optimizer, epoch, num_updates, args.lr)
                else:
            ...
            for i, batch in loop: # train_loader length = 48
                ...
                with torch.no_grad(): # disable gradient calculation, save memory, more efficient
                    ...
                    if args.distributed:
                        scores, critic_inputs = agent.module.actor_critic(input_var)
                    else:
                ...
                # has num_mini_batchs,
                if args.distributed:
                    value_loss, action_loss, dist_entropy, losses = agent.module.update(rollouts, drug_idx_to_original_map, do_constrained=do_constrained, device=device, distributed=True)
                else:
                    value_loss, action_loss, dist_entropy, losses = agent.update(rollouts, drug_idx_to_original_map, do_constrained=do_constrained, device=device, distributed=False) #calc standardised advantages, does PPO updates, returns losses
            ...
            print('calling validate with constrained (update lambda) as', do_constrained)
            ...
            if is_best and not args.Debug: #if the model is the best and not in debug mode
                ...
                with torch.no_grad(): #disable gradient calculation, save memory, more efficient
                    ...
                    if args.distributed:
                        Y_pred, _ = agent.module.actor_critic(test_input_var)
                    else:
            ...
            if (epoch % args.save_interval == 0 or epoch == num_updates - 1) and args.saved_dir != "": #save the model every save_interval epochs or at the end of training
                if args.distributed:
                    utils.save_checkpoint({  # save the model
                        'epoch': epoch,
                        'Policy_state_dict': agent.module.actor_critic.state_dict(),
                        'best_ndcg': best_ndcg,
                        'optimizer': agent.module.optimizer.state_dict(),
                    }, is_best, model_save_dir, filename='/checkpoint.{0:03d}_device{1}.tar'.format(epoch, int(local_rank)))
                else:
                    utils.save_checkpoint({ #save the model
                        'epoch': epoch,
