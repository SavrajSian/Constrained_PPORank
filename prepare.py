...
def keepk_sample(Y_train, Y_test, keepk_ratio, keepk): #create a subset of the data where only the top k most effective drugs are kept for each cell line in the training data.
    ...
    for i in range(Y_train_keepk.shape[0]):
        ...
        y_argsort_pos = y_argsort[:N] #top N drugs
        y_argsort_neg = y_argsort[N:] #remaining drugs
        pos_permutation = np.random.permutation(y_argsort_pos.shape[0]) #randomly permute the top N drugs
        for j in range(keepk, pos_permutation.shape[0]):
            y[y_argsort_pos[pos_permutation[j]]] = np.nan #set values at permuted indices (beyond keepk) to nan
        neg_permutation = np.random.permutation(y_argsort_neg.shape[0])
        for j in range(0, neg_permutation.shape[0]):
            y[y_argsort_neg[neg_permutation[j]]] = np.nan    #set values at permuted indices to nan
    ...
    Y_train_keepk = pd.DataFrame(Y_train_keepk, index=inds_train, columns=cols) # (788,215) df with keepk drugs
    Y_test_keepk = pd.DataFrame(Y_test_keepk, index=inds_test, columns=cols) #test df with keepk drugs
