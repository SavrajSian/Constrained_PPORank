...
def validate(agent, test_loader, args, device, drug_idx_to_original_map=None, rollouts=None, update_lambda=False):
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            if args.algo == 'ppo':
                if dist:
                    output, _ = agent.module.actor_critic(input_var)  # .squeeze().type(torch.DoubleTensor)
                else:
                ...
                if dist:
                    output = agent.module.policy_net(input_var).squeeze().type(torch.DoubleTensor)
                else:
        ...
        print('in eval, update_lambda:', update_lambda)
        if update_lambda == True:
            ...
            ranks = torch.arange(1, len(preds_order) + 1, dtype=torch.float, device=device)
...
def evaluation(agent, data_loader, device, resume_file, algo, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(data_loader):
            ...
            if algo == 'ppo':
                if dist:
                    output = agent.module.actor_critic(input_var).squeeze().type(torch.DoubleTensor)
                else:
