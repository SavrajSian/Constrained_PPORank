    ...
    def sample_num_steps(self, agent, input, scores, true_scores, critic_inputs, args):
        ...
        for cell in range(num_process):
            ...
            for step in range(M0):
                self.filter_masks[step+self.steps].copy_(filter_masks)
                if dist:
                    selected_drug_id, _ = agent.module.actor_critic.sample_action(scores[cell], filter_masks)
                else:
                ...
                if dist:
                    log_prob, dist_entropy = agent.module.actor_critic.get_log_prob(scores[cell], filter_masks, selected_drug_id) #prob of action. filter mask adding -inf to score means it cant be selected
                else:
                ...
                if dist:
                    ob_critic = agent.module.actor_critic.get_fts_vecs(input[cell], filter_masks)
                else:
                ...
                if dist:
                    value_pre = agent.module.actor_critic.get_value_from_actor(critic_inputs[cell].unsqueeze(0))
                else:
